"""
simulate_2025_trio.py
=====================
2025年レース限定: 3連複・3連単 プールサイズ5頭 vs 7頭 シミュレーション。

- テスト期間  : 2025-01-01 〜 2025-12-31
- 対象        : 買い条件通過レース（daily_batch.py と同一フィルター）
- 比較        : プール5頭/5点（変更前） vs プール7頭/7点（変更後）
- 3連複オッズ : Harville 法で推定（JRA 控除率 22.5%）
- 3連単オッズ : Harville 法で推定（JRA 控除率 27.5%）

実行:
    .venv/bin/python simulate_2025_trio.py
"""
...
logger.add("logs/simulate_2025_trio.log", level="DEBUG", format=_fmt, rotation="10 MB")
...
def build_combos(pool, partner_pred_idxs, model_probs, mkt_probs, honmei_idx,
                 ticket_n, bet_type):
    """プールから EV 上位 ticket_n 点を選択"""
    all_items = []
    if bet_type == "3連複":
        for (na, via), (nb, vib) in _comb(pool, 2):
            pa = partner_pred_idxs.get(na, 1)
            pb = partner_pred_idxs.get(nb, 2)
            model_p = _prob_trio(model_probs, honmei_idx, pa, pb)
            if via is not None and vib is not None:
                mkt_p = _prob_trio(mkt_probs, honmei_idx, via, vib)
                e_od  = _est_odds(mkt_p, "3連複")
            else:
                e_od  = _est_odds(model_p, "3連複")
            all_items.append((na, nb, model_p * e_od, e_od))
    else:  # 3連単
        for (n2, vi2), (n3, vi3) in _perm(pool, 2):
            p2 = partner_pred_idxs.get(n2, 1)
            p3 = partner_pred_idxs.get(n3, 2)
            model_p = _prob_sanrentan(model_probs, honmei_idx, p2, p3)
            if vi2 is not None and vi3 is not None:
                mkt_p = _prob_sanrentan(mkt_probs, honmei_idx, vi2, vi3)
                e_od  = _est_odds(mkt_p, "3連単")
            else:
                e_od  = _est_odds(model_p, "3連単")
            all_items.append((n2, n3, model_p * e_od, e_od))

    all_items.sort(key=lambda x: -x[2])
    sel = all_items[:ticket_n]
    est_ods = [od for *_, od in sel]
    if est_ods and _synth_odds(est_ods) < 1.0:
        return []
...
def simulate() -> None:
    t0 = time.time()
    logger.info("=" * 65)
    logger.info(f"2025年 3連複・3連単 プール5 vs 7 シミュレーション開始")
    logger.info("=" * 65)
    ...
    test_meta = test_meta.dropna(subset=["race_date"])
    test_meta = test_meta[test_meta["race_date"].dt.year == SIM_YEAR].sort_values("race_date")
    logger.info(f"2025年レース数: {len(test_meta):,}  ({test_meta['race_date'].min().date()} 〜 {test_meta['race_date'].max().date()})")
                ...
                if bet_type == "3連複":
                    for na, nb in combos:
                        if frozenset([honmei_num, na, nb]) == actual_trio:
                            ...
                            if via is not None and vib is not None:
                                mp = _prob_trio(mkt_probs, honmei_idx, via, vib)
                                hit_od = _est_odds(mp, "3連複")
                            break
                else:  # 3連単
                    if honmei_num == pos1_num:
                        for n2, n3 in combos:
                            if n2 == pos2_num and n3 == pos3_num:
                                ...
                                if vi2 is not None and vi3 is not None:
                                    mp = _prob_sanrentan(mkt_probs, honmei_idx, vi2, vi3)
                                    hit_od = _est_odds(mp, "3連単")
                                break
    ...
    border = "=" * 68
    print(f"\n{border}")
    print(f"  2025年 3連複・3連単 プール5頭 vs 7頭 シミュレーション結果")
    ...
    for bet_type in ["3連複", "3連単"]:
        print(f"\n  ▼ {bet_type}")
        print(f"  {'':8}  {'投票額':>9}  {'推定回収':>9}  {'ROI':>7}  {'的中数':>8}  {'的中率':>7}  {'平均点':>6}")
        for pool_n, ticket_n in zip(POOL_SIZES, TICKET_COUNTS):
            key = f"p{pool_n}_{bet_type}"
            total_cost = df[f"{key}_cost"].sum()
            total_ret  = df[f"{key}_ret"].sum()
            hits       = df[f"{key}_hit"].sum()
            roi        = total_ret / total_cost * 100 if total_cost > 0 else 0
            avg_combo  = df[f"{key}_combos"].mean()
            label = f"プール{pool_n}頭/{ticket_n}点"
            print(f"  {label:10}  {int(total_cost):>8,}円  {int(total_ret):>8,}円  {roi:>6.1f}%"
                  f"  {int(hits):>3}/{len(df)}R  {hits/len(df):>6.2%}  {avg_combo:>5.1f}点")

    # ── 拡張効果（差分）────────────────────────────────────────
    print(f"\n  ▼ 拡張効果（プール5→7）")
    print(f"  {'':10}  {'新規捕捉':>6}  {'追加コスト':>10}  {'追加回収':>9}  {'追加ROI':>8}")
    for bet_type in ["3連複", "3連単"]:
        key5 = f"p5_{bet_type}"
        key7 = f"p7_{bet_type}"
        ...
        print(f"  {bet_type:10}  {len(newly):>5}R  {int(extra_cost):>9,}円  {int(extra_ret):>8,}円  {extra_roi:>7.1f}%")
    ...
    for bet_type in ["3連複", "3連単"]:
        ...
        key7 = f"p7_{bet_type}"
        print(f"\n  ▼ 月別 ROI [{bet_type}]")
        print(f"  {'月':>4}  {'R数':>4}  {'1着率':>7}  {'pool5':>9}  {'pool7':>9}  {'差':>7}  {'pool5的中':>8}  {'pool7的中':>8}")
        for month, grp in df.groupby("race_month"):
            c5 = grp[f"{key5}_cost"].sum()
            r5 = grp[f"{key5}_ret"].sum()
            c7 = grp[f"{key7}_cost"].sum()
            r7 = grp[f"{key7}_ret"].sum()
            roi5 = r5 / c5 * 100 if c5 > 0 else 0
            roi7 = r7 / c7 * 100 if c7 > 0 else 0
            h5 = int(grp[f"{key5}_hit"].sum())
            h7 = int(grp[f"{key7}_hit"].sum())
            hon_rate = grp["honmei_1st"].mean()
            print(f"  {month:>4}月  {len(grp):>4}R  {hon_rate:>6.1%}  "
                  f"{roi5:>8.1f}%  {roi7:>8.1f}%  {roi7-roi5:>+7.1f}pt  "
    ...
    for bet_type in ["3連複", "3連単"]:
        ...
        print(f"  {'gap帯':>10}  {'R数':>4}  {'1着率':>7}  {'pool5':>9}  {'pool7':>9}  {'差':>7}")
        for gb, grp in df.groupby("gap_bin", observed=True):
            ...
            roi7 = r7 / c7 * 100 if c7 > 0 else 0
            hon_rate = grp["honmei_1st"].mean()
            print(f"  {str(gb):>10}  {len(grp):>4}R  {hon_rate:>6.1%}  "
                  f"{roi5:>8.1f}%  {roi7:>8.1f}%  {roi7-roi5:>+7.1f}pt")
    ...
    for bet_type in ["3連複", "3連単"]:
        ...
        if len(newly) > 0:
            ...
            for _, r in newly.iterrows():
                actual_disp = r["actual_trio"] if bet_type == "3連複" else r["actual_st"]
                print(f"    {r['race_date']} {r['race_name'][:20]:20s}  "
                      f"実際: {actual_disp}  推定配当: {r[f'{key7}_hit_od']:.0f}倍  "
                      f"◎1着: {'○' if r['honmei_1st'] else '×'}")

    print(f"\n{border}\n")

    # 保存
    out = ROOT / "data/processed/simulate_2025_trio.csv"
