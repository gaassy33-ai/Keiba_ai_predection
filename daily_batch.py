...
# 定数
# ======================================================================
MIN_HONMEI_PROB      = 0.30   # ○最低勝率（以下は△）バックテスト結果: 25-30%帯ROI71%のため引き上げ
MARK_STRONG_PROB     = 0.40   # ◎最低勝率（○との境界）MIN_HONMEI_PROB引き上げに合わせてシフト
MIN_CONFIDENCE_GAP   = 0.05   # 勝率差フィルター（以下は△）
# EV フィルタ（model_prob × odds ≥ 閾値 のみ買い）
# 評論家フィードバック: 1.05→1.15 に強化（より確実なバリュー馬のみ選別）
EV_THRESHOLD         = 1.15   # 15% のポジティブエッジを要求
